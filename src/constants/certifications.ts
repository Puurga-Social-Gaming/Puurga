/**
 * Puurga certification catalog (managed / granted by Super Admin).
 * Premium checks can stack with the Official logo badge.
 */

export type CertificationSlug = 'blue' | 'gold' | 'business' | 'elite';
export type CertificationKind = 'check' | 'logo';
...
export interface CertificationType {
  ...
  shortTitle: string;
  color: string;
  /** Price in Puurga points / credits (0 = not for sale) */
  price: number;
  kind: CertificationKind;
  description: string;
  /** Soft eligibility hints for admins */
  minPoints?: number;
  minPosts?: number;
...
export const PREMIUM_CHECK_SLUGS: CertificationSlug[] = ['blue', 'gold', 'business', 'elite'];
